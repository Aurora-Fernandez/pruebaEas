...
public class RaAgency
    ...
    class ReceiveMessageThread extends Thread implements Serializable {
        ...
        public void run() {
            ...
            try {
                ...
                if (!message.recipient.host.equals(agencyAddress.host)) {
                ...
                } else if (message.kind.equals("RA")) {
                    ...
                    System.out.println(
                            "ReceiveMessageThread: Ha llegado un mensaje GET_CLASS: " + message.content + ".class");
    ...
    class ListenThread extends Thread implements Serializable {
        ...
        public void run() {
            Thread shouldLiveThread = listenThread;
            try {
                while (shouldLiveThread == listenThread) {
                    socket = serverSocket.accept();
                    if (shouldLiveThread != listenThread) {
                        return;
                    }
    ...
    protected class SendMessageThread extends Thread implements Serializable {
        ...
        public void run() {
            ...
            try {
                ...
                System.out.println(
                        "SendMessageThread: socket created to: " + msg.recipient.host + " " + msg.recipient.port);
                outStream = new ObjectOutputStream(
                        socket.getOutputStream());
    ...
    public Hashtable getServers(Object sender) {
        ...
        synchronized (this) {
            if (agencys != null)
                result = (Hashtable) agencys.clone();
        }
        return result;
    }

    public synchronized void addAgencyListener(AgencyListener l) {
        if (agencyListeners.contains(l))
            return;
    ...
    public void raMessage(RaMessageEvent e) {
        ...
        if (message != null) {
            if (message.recipient != null) {
                if (message.recipient.host == null) { // localhost
                    RaBox box = null;
                    ...
                    if (box != null) {
                        ...
                        System.err.println("No es un agente local " + message.recipient.name);
                } else {
                    new SendMessageThread(message).start();
                }
            }
