...
public class RaClassLoader extends SecureClassLoader {
    ...
    protected byte loadClassData(String name)[]
        ...
        try {
            ...
            System.out.println("el cargador ha establecido la conexi�n");
    ...
    public Class findClass(String name)
        ...
        if (data == null)
            throw new ClassNotFoundException(name);
        try {
            System.out.println("Intentando definir la Clase: " + name + " tama�o: " + data.length);

            URL srcURL = new URL("http", sourceHost.host.getHostAddress(), sourceHost.port, "/");
            CodeSigner[] codeSigners = null;
            CodeSource codeSrc = new CodeSource(srcURL, codeSigners);
